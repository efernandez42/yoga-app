...
/**
 * Tests d'intégration pour le composant d'inscription
...
describe('RegisterComponent Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      imports: [
        RouterTestingModule.withRoutes([{ path: 'login', redirectTo: '' }]), // Module de test pour le routing
  ...
  // Test d'intégration : Affichage du formulaire et inscription réussie
  it('should display register form and handle successful registration flow', async () => {
    ...
    const firstNameInput: HTMLInputElement = fixture.debugElement.query(By.css('input[formControlName="firstName"]')).nativeElement;
    const lastNameInput: HTMLInputElement = fixture.debugElement.query(By.css('input[formControlName="lastName"]')).nativeElement;
    ...
    firstNameInput.dispatchEvent(new Event('input')); // Déclenche l'événement de saisie
    lastNameInput.value = 'Doe';
    lastNameInput.dispatchEvent(new Event('input')); // Déclenche l'événement de saisie
    ...
    // Vérification de l'appel HTTP à l'API d'inscription
    ...
    req.flush(null); // Simule une réponse réussie
  ...
  it('should handle registration error and display error message', async () => {
    ...
    // Simulation de la saisie avec des données valides
    emailInput.value = 'test@test.com';
    ...
    // Simulation d'une erreur HTTP 400 (Mauvaise requête)
    const req = httpTestingController.expectOne('api/auth/register');
    expect(req.request.method).toBe('POST');
  ...
  it('should validate form fields and disable submit when invalid', async () => {
    ...
    // Test avec des noms trop courts
    emailInput.value = 'test@test.com';
    emailInput.dispatchEvent(new Event('input'));
    firstNameInput.value = 'Jo'; // Trop court (minimum 3 caractères)
    firstNameInput.dispatchEvent(new Event('input'));
    lastNameInput.value = 'Do'; // Trop court (minimum 3 caractères)
    lastNameInput.dispatchEvent(new Event('input'));
    passwordInput.value = 'password123';
    passwordInput.dispatchEvent(new Event('input'));
    fixture.detectChanges();
